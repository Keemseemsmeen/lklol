...
if (!(_SelectedTransporterTypeS) && !(_SelectedTransporterTypeM) && !(_SelectedTransporterTypeL) && !(_SelectedTransporterTypeXL)) exitWith {
	...
	hint parseText format ["
		...
		<img color='#ffffff' image='loki_lost_key\BTK\Cargo Drop\Images\img_line_ca.paa' align='left' size='0.79' />
...
if ((_Selected == "LoadCargo") && (_SelectedTransporterTypeS)) exitWith {
	...
	_UnloadAction = _Transporter addAction [("<t color=""#fadfbe"">" + ("Unload cargo") + "</t>"),"\loki_lost_key\BTK\Cargo Drop\Engine.sqf",["UnloadCargo"], 5];
