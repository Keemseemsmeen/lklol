#include "defs.hpp"
#define GET_DISPLAY (findDisplay loki_debug_VC_IDD)
...
case 0: {
		...
		lbClear GET_CTRL(loki_VC_vehlist_IDC);
		for "_i" from 0 to (count _cfgvehicles)-1 do {
			...
			if (isClass _vehicle) then {
				...
				if ((getNumber(_vehicle >> "scope")==2)and(getText(_vehicle >> "picture")!="")and(KINDOF_ARRAY(_veh_type,_kindOf))and!(KINDOF_ARRAY(_veh_type,_filter))) then {
					GET_CTRL(loki_VC_vehlist_IDC) lbAdd (getText(_vehicle >> "displayName"));
					GET_CTRL(loki_VC_vehlist_IDC) lbSetData [(lbSize GET_CTRL(loki_VC_vehlist_IDC))-1,_veh_type];
					GET_CTRL(loki_VC_vehlist_IDC) lbSetPicture [(lbSize GET_CTRL(loki_VC_vehlist_IDC))-1,getText(_vehicle >> "picture")];
		...
		lbSort GET_CTRL(loki_VC_vehlist_IDC);
	};
case 1: {
		_veh_type = GET_SELECTED_DATA(loki_VC_vehlist_IDC);
		GET_CTRL(loki_VC_vehicle_shortcut_IDC) ctrlSetText (getText(configFile >> "CfgVehicles" >> _veh_type >> "picture"));
		...
		GET_CTRL(loki_VC_veh_info_IDC) ctrlSetStructuredText _text;
	};
case 2: {
		_old_veh = loki_get(VEH);
		_core = loki_get(CORE);
		_dir = getDir _core;
		_pos = getPos _core;
		_veh_type = GET_SELECTED_DATA(loki_VC_vehlist_IDC);
...
case 4: {//class to clipboard
	copyToClipboard (""""+GET_SELECTED_DATA(loki_VC_vehlist_IDC)+"""");
	};

case 5: {//detailed info to clipboard
	copyToClipboard ctrlText GET_CTRL(loki_VC_veh_info_IDC);
	};
}
