/*****************************************************************************
  ...
  Description:
  Fills the values and parents list boxes
  
  Input:
	_this select 0: config
	_this select 1: value names
	_this select 2: parent level
	_this select 3: code string (optional)
  Returns:
	nothing
*****************************************************************************/
...
_newLine = loadfile "\loki_lost_key\proving_ground\CfgExplorer2\scripts\NewLine.sqf";
...
for "_y" from 0 to ((count _curConfig) - 1) do {	
	...
	if (not (isClass _Entry) && (not ((toUpper _EntryName) in _valueNames))) then { 
		...
		if (isArray _Entry) then { 
			...
			_index = lbAdd [111, _text];
			_ArrayString = [getArray _Entry] call hj_fn_ArrayToString;
...
if (format["%1", inheritsFrom _curConfig] != "") then
{
	_Code = [inheritsFrom _curConfig, _valueNames, (_parentLevel + 1), _Code ] call hj_fn_fillValues;
};
