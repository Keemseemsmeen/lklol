#include "defs.hpp"
#define GET_DISPLAY (findDisplay loki_debug_WC_IDD)
...
case 0: {//fill weapon list
		...
		lbClear GET_CTRL(loki_WC_weaplist_IDC);
		for "_i" from 0 to (count _cfgweapons)-1 do {
			...
			if (isClass _weapon) then {
				...
				((_item_type in [3,4,5,6,7])||((_item_type==0)&&(_no_pack!=1)&&((_optics=="-")))||((_item_type==1)&&(_no_pack!=1)&&((_optics!="-")))||((_item_type==2)&&((_cur_type==5)||((_no_pack==1)&&(_cur_type in _type)))))) then {
					GET_CTRL(loki_WC_weaplist_IDC) lbAdd _display_name;
					GET_CTRL(loki_WC_weaplist_IDC) lbSetData [(lbSize GET_CTRL(loki_WC_weaplist_IDC))-1,_weap_type];
					GET_CTRL(loki_WC_weaplist_IDC) lbSetPicture [(lbSize GET_CTRL(loki_WC_weaplist_IDC))-1,getText(_weapon >> "picture")];
		...
		lbSort GET_CTRL(loki_WC_weaplist_IDC);		


	};
case 1: {//weap info, fill magazines
		_weap_type = GET_SELECTED_DATA(loki_WC_weaplist_IDC);
		...
		GET_CTRL(loki_WC_weapon_shortcut_IDC) ctrlSetText (_picture);
		_lb = parseText "<br/>";
		_text = composeText ["Class: ",str configName(_weapon),_lb,
			"DisplayName: ",str _displayName,_lb,
			"Dispersion: ",str _dispersion,_lb,_lb,
			 parseText _library];
		GET_CTRL(loki_WC_weap_info_IDC) ctrlSetStructuredText _text;
		lbClear GET_CTRL(loki_WC_magazinelist_IDC);
		{
			GET_CTRL(loki_WC_magazinelist_IDC) lbAdd (getText(configFile >> "cfgMagazines" >> _x >> "displayName"));
			GET_CTRL(loki_WC_magazinelist_IDC) lbSetData [(lbSize GET_CTRL(loki_WC_magazinelist_IDC))-1,configName(configFile >> "cfgMagazines" >> _x)];
			GET_CTRL(loki_WC_magazinelist_IDC) lbSetPicture [(lbSize GET_CTRL(loki_WC_magazinelist_IDC))-1,getText(configFile >> "cfgMagazines" >> _x >> "picture")];
		} forEach _magazines;
...
case 3: {//ammo info
		_mag = GET_SELECTED_DATA(loki_WC_magazinelist_IDC);
		...
		GET_CTRL(loki_WC_magazine_info_IDC) ctrlSetStructuredText _text;
	};
...
case 5: {//weap to clipboard
	copyToClipboard (""""+GET_SELECTED_DATA(loki_WC_weaplist_IDC)+"""");
	};

case 6: {//ammo to clipboard
	copyToClipboard (""""+GET_SELECTED_DATA(loki_WC_magazinelist_IDC)+"""");
	};
};