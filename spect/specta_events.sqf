			...
			if (_param select 0) then {
				...
				_cam = _param select 1;
				{
					...
					if (alive _u) then {
						...
						_s setParticleParams ["\loki_lost_key\spect\tag.p3d", "", "billboard", 1, _lifeTime, [0, 0, 2], [0, 0, 0], 1, 1, 0.784, 0.1, [_size, _size*0.66], [_color, _color, _color, _color, _colorB], [1], 10.0, 0.0, "", "", vehicle _u];
